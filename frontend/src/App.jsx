...
function App() {
    ...
    useEffect(() => {
        ...
        const updateActivity = async (status = 'online') => {
            const token = Cookies.get('synapse_token');
            if (!token) {
                console.log('⏳ [Global Activity] Missing token, skipping pulse...');
                return;
            }
            ...
            try {
                const response = await fetch(`${LIVE_API}/api/user/activity`, {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    console.log(`📡 [Global Activity] ${status.toUpperCase()} Pulse sent successfully`);
                }
            } catch (err) {
                console.log("Global Activity Update Error:", err);
            }
        };
